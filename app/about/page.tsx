...
import { about, team } from "@/lib/mock/about";
...
const volunteers = team.filter((m) => !m.isLeadership);
    const shuffled = [...volunteers].sort((a, b) => a.name.localeCompare(b.name));
const preview = [...leaders, ...shuffled].slice(0, PREVIEW_COUNT);
    ...
    <>
        ...
        <div className="mx-auto grid max-w-6xl items-stretch gap-10 px-5 pb-16 pt-32 md:grid-cols-2 md:gap-12 md:pb-20 md:pt-40">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-x-10 gap-y-4">
              ...
              {/* <Meta label="Status" value={about.registrationStatus} /> */}
            </div>
          </div>

          {/* photo — clean, no fades */}
      ...
      <section className="mx-auto max-w-6xl px-5 py-20 md:py-28">
        <div className="grid items-center gap-8 md:grid-cols-[1.5fr_1fr] md:gap-16">
          ...
          {/* right — the payoff, vertically centered, bigger */}
        ...
        <div className="mx-auto max-w-6xl px-5">
          <div className="grid gap-12 border-t border-ink/10 pt-12 md:grid-cols-[1.6fr_1fr] md:gap-16 md:pt-16">
            ...
            <div className="md:pt-16">
              ...
              <p className="type-lead mt-5 text-ink/70">{about.vision}</p>
            </div>
          </div>
        </div>
      </section>
...
{/* leadership — named cards, credentialed core only */}
<section className="bg-paper py-20 md:py-28">
  <div className="mx-auto max-w-6xl px-5">
    <div className="max-w-2xl">
      <p className="type-caption text-tally">Leadership</p>
      <h2 className="type-h2 mt-3 text-ink">The team behind the work.</h2>
      <p className="type-lead mt-4 text-ink/60">
        The founders, clinicians and coordinators leading every outreach.
      </p>
    </div>
      ...
      {leaders.map((m, i) => (
        <li key={i}>
          <div className="relative aspect-4/5 overflow-hidden rounded-xl border border-ink/10 bg-ink/3">
            {m.image ? (
              <Image src={m.image} alt={m.name} fill className="object-cover" sizes="(max-width:768px) 50vw, 25vw" />
            ) : (
              <div className="flex h-full items-center justify-center">
                <span className="font-display text-4xl text-ink/15">CC</span>
              </div>
            )}
          </div>
          ...
          {m.credential && <p className="type-caption mt-1 text-tally">{m.credential}</p>}
        </li>
      ))}
    </ul>
  </div>
