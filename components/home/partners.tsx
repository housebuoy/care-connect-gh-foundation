  ...
  return (
    <section className="border-y border-ink/10 bg-paper py-14 md:py-16">
      <div className="mx-auto max-w-6xl px-5">
        <p className="type-caption text-center text-tally">
          Our Partners
        </p>
      </div>
...
function PartnerMark({ partner }: { partner: import("@/lib/mock/partners").Partner }) {
  const content = partner.logo ? (
    <div className="relative h-10 w-32 opacity-60 grayscale transition hover:opacity-100 hover:grayscale-0 md:h-12 md:w-40">
      <Image src={partner.logo} alt={partner.name} fill className="object-contain" sizes="160px" />
    </div>
  ) : (
  ...
  return partner.url ? (
    <a href={partner.url} target="_blank" rel="noopener noreferrer">{content}</a>
  ) : (
    content
  );
}